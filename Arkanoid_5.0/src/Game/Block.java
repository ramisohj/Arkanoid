...
public class Block {
    ...
    private void crashCorner(Ball ball, int x1, int y1, int x2, int y2){
        double n1 = Math.pow(x1-ball.getXC(),2)+Math.pow(y1-ball.getYC(),2)-Math.pow(ball.getRadio(),2);
        double n2 = Math.pow(x2-ball.getXC(),2)+Math.pow(y2-ball.getYC(),2)-Math.pow(ball.getRadio(),2);
        if(n1<=0 || n2<=0){
            ball.setDX(ball.getDX()*-1);
            ball.setDY(ball.getDY()*-1);
        }
